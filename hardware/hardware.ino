...
void loop() {
  ...
  int acquiredValue = analogRead(analogPin);
  int value = map(acquiredValue, 0, 4095, 0, 9);
...
void checkPasscode(void) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin(client, "http://172.16.194.10:8080/api/check/combination");  // Your Domain name with URL path or IP address with path
